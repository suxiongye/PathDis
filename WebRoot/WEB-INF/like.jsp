				...
				if (type == 0) {
					...
					if (path1 != null && path2 != null && tracks != null) {
						...
						for (i in tracks) {
							...
							if (tracks[i] != "") {
								var points = tracks[i].split(',');
								var path1Tracks = [];
								var path2Tracks = [];
								for (j in points) {
									var ps = points[j].split(':');
									console.log(ps[0]);
									if (points[j] != "") {
										var point1 = new BMap.Point(
												path1[ps[0]].longitude,
												path1[ps[0]].latitude);
										path1Tracks.push(point1);
										var point2 = new BMap.Point(
												path2[ps[1]].longitude,
												path2[ps[1]].latitude);
										path2Tracks.push(point2);
									}
								}
								var polyline3 = new BMap.Polyline(path1Tracks,
										{
											strokeColor : "#000000",
											strokeWeight : 3,
											strokeOpacity : 1
								...
								map.addOverlay(polyline3);
								var polyline4 = new BMap.Polyline(path2Tracks,
										{
											strokeColor : "#000000",
											strokeWeight : 3,
											strokeOpacity : 1
